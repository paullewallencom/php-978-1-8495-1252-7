...
set_include_path(KFM_BASE_PATH.'classes'.PATH_SEPARATOR.get_include_path());
...
$kfm->defaultSetting('max_image_upload_width', 1601);
$kfm->defaultSetting('max_image_upload_height', 1601);
// plugins
...
if (!$kfm_session->get('loggedin') && (!isset($kfm_api_auth_override)||!$kfm_api_auth_override)) {
    ...
    if (isset($_POST['username'])&&isset($_POST['password'])) {
        $res=db_fetch_row('SELECT id, username, password, status FROM '.KFM_DB_PREFIX.'users WHERE username="'.mysql_escape_string($_POST['username']).'" AND password="'.sha1($_POST['password']).'"');
        if($res && count($res)){

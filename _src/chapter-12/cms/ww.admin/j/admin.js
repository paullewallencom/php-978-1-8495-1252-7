...
function typeOf(value) { // from http://javascript.crockford.com/remedial.html
	var s = typeof value;
	if (s === 'object') {
		if (value) {
...
function json_encode(obj){
	switch(typeOf(obj)){
		...
		case 'object':
			var string=[];
			for(var property in obj)string.push(json_encode(property)+':'+json_encode(obj[property]));
			return '{'+string.join(',')+'}';
		case 'number':
